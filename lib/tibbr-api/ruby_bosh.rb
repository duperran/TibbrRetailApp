# https://github.com/skyfallsin/ruby_bosh/raw/master/lib/ruby_bosh.rb
# Unable install the gem on Jruby so made a local copy
...
class RubyBOSH
  ...
  def connect
    ...
    #any step failure is considered as failure
    if send_auth_request and send_restart_request and request_resource_binding
      @success = send_session_request
    end

    unless success?
      Rails.logger.info("Error while authenticating the chat session for jid #{@jid}")
      return [nil, nil, nil, @latest_response]
    end

    @rid += 1 #updates the rid for the next call from the browser

    [@jid, @sid, @rid, @latest_response]
  end
    ...
    request = construct_body(:sid => @sid) do |body|
      body.iq(:id => "bind_#{rand(100000)}", :type => "set",
        ...
        iq.bind(:xmlns => BIND_XMLNS) do |bind|
          #assign a resource value passed by calling code if it is present
          bind.resource((@jid.split("/").last.strip.presence || "bosh_#{rand(10000)}"))
        end
      end
    end
